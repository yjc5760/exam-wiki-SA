...
*圖說（原卷截圖）：門型剛構架。左柱底 A、右柱底 D 固定；頂部節點 B（左）、C（右）剛接連接水平梁 BC。所有桿件撓曲剛度 $EI$、長度 $L$。側向力 $P$ 作用於梁端（水平向右）。*

![向量重繪：門型剛構架，節點 A/B/C/D、EI 與 L 標註、固定支承符號、側向力 P](../../raw/solutions/SA-2025-1/figs/SA-2025-1-fig-1-frame.svg)

*圖 1R　向量重繪版（作答時可照此比例快速描繪）*
...
![自由度辨識圖解：保留 θB、θC、Δ 三個自由度，並標示被忽略軸向變形消去的垂直位移與水平位移耦合](../../raw/solutions/SA-2025-1/figs/SA-2025-1-fig-2-dof.svg)
...
![勁度矩陣三行的物理意義：三個單位位移狀態及其所需節點力](../../raw/solutions/SA-2025-1/figs/SA-2025-1-fig-3-unit-states.svg)

*圖 3　勁度矩陣每一行的物理意義。第 j 行 ＝ 讓第 j 個自由度產生單位位移、其餘全部鎖住時，各自由度上必須施加的力。核對矩陣正負號時直接看這張圖，不必重推公式。*
...
![變形形狀與彎矩圖：柱雙曲率、反曲點在距柱底 4L/7 處，梁反對稱、中點彎矩為零](../../raw/solutions/SA-2025-1/figs/SA-2025-1-fig-4-deflected-bmd.svg)

*圖 4　左：側移模式之變形形狀，柱反曲點位於距柱底 $4L/7$（由 $M_{AB}:M_{BA}=2/7:3/14$ 線性內插得到），梁中點為反曲點。右：繪於受拉側之彎矩圖。兩張圖可作為答案的獨立檢核——若算出的反曲點不在 $4L/7$、或梁中點彎矩不為零，前面必有錯。*
...
（相當於梁無限剛時之柱剪切勁度的 70%，其餘 30% 由節點旋轉造成剛度折減。）

![側向勁度光譜：梁無限剛 24EI/L³、本題 16.8EI/L³、梁無勁度 6EI/L³](../../raw/solutions/SA-2025-1/figs/SA-2025-1-fig-5-stiffness-spectrum.svg)

*圖 5　側向勁度光譜。靜態凝縮扣掉的 $-36EI/5L^3$ 就是「節點被允許旋轉」的代價。本題答案必定落在 $6EI/L^3$（兩根獨立懸臂柱）與 $24EI/L^3$（梁無限剛）之間——這是最快的量級檢核。*
...
*解析日期：2026-07-02　｜　圖解補繪：2026-08-10（向量圖，產生腳本 `gen_sa2025_1.py` + `structdraw.py`，可重跑）*


[互動圖形預覽](../../raw/solutions/SA-2025-1/SA-2025-1-frame-viz.html)
