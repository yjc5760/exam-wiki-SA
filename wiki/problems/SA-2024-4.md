...
![SA-2024-4 題目附圖：門型剛架，A、D 固定端，柱 AB 與 CD 長 2L、剛度 2EI，梁 BC 長 L、剛度 EI，P 作用於 B 點向右](SA-2024-4-fig-1.png)

*圖說：門型剛架。$A$、$D$ 為固定端。左柱 $AB$ 與右柱 $CD$ **長 $2L$、剛度 $2EI$**；橫梁 $BC$ **長 $L$、剛度 $EI$**。集中力 $P$ 作用於節點 $B$，方向向右。（以上幾何與剛度皆由考卷附圖直接標註讀出。）*

![SA-2024-4 題目重繪：三根桿的相對勁度 K = EI/L 完全相等](figs/SA-2024-4-fig-1-frame.svg)

*圖 1　題目重繪（向量版）。這張圖攔的是「柱 $2EI$、梁 $EI$ 看起來不對稱」的第一印象：換算成相對勁度 $K = EI/L$ 之後，三根桿完全相等 —— 這是出題者刻意的設計，先看出來就能少算一半。*
...
![SA-2024-4 反對稱變形與彎矩圖：兩柱皆為雙曲率且變形模式相同，柱底 4/7 PL 是柱頂 3/7 PL 的 4/3 倍](figs/SA-2024-4-fig-2-antisym-bmd.svg)

*圖 2　反對稱變形與彎矩圖。這張圖攔兩件事：以為柱頂與柱底的彎矩一樣大（柱底 $\frac47 PL$ 是柱頂 $\frac37 PL$ 的 $\frac43$ 倍，因為柱頂還要跟梁分攤節點彎矩），以及把「反對稱」誤畫成鏡像 —— 兩柱的變形模式是**完全相同**，不是左右對照。*

| 量 | 值 | 說明 |
|---|---|---|
| $\Delta_B$ | $\dfrac{5}{21}\dfrac{PL^3}{EI}$（向右） | B、C 同一個側移 |
| $\theta_B = \theta_C$ | $\dfrac{1}{14}\dfrac{PL^2}{EI}$（順時針） | 反對稱 $\Rightarrow$ 兩節點轉角相同 |
| $M_{AB} = M_{DC}$ | $\dfrac47 PL$ | **柱底** |
| $M_{BA} = M_{CD} = M_{BC} = M_{CB}$ | $\dfrac37 PL$ | **柱頂與梁端** |
| $H_A = H_D$ | $P/2$ | 剛度相同 $\Rightarrow$ 平分層剪力 |

> **驗算：** 節點 $B$、$C$ 的力矩平衡皆為 0；層剪力 $\dfrac{M_{AB}+M_{BA}}{2L} + \dfrac{M_{CD}+M_{DC}}{2L} = \dfrac{4/7+3/7}{2} P \times 2 = P$ ✓
...
|---|---|---|
| 2026-07-02 | 初版解析 | — |
| 2026-08-26 | **補上 §1 缺漏的考卷附圖引用與圖說**（原文以「（註：經確認幾何配置與剛度為…）」帶過，未引用 `SA-2024-4-fig-1.png`）；補兩張向量圖；主分類名稱與 `question_index.json` 對齊 | 附圖上的 `2EI / 2L / EI / L` 標註本來就清楚，不需要「經確認」的括號註記。所有數值（$\Delta_B = \frac{5}{21}\frac{PL^3}{EI}$、$M_{CD} = \frac37 PL$、$M_{DC} = \frac47 PL$）經剛架有限元驗證**全部正確** |

---

*解析日期：2026-07-02　｜　補圖：2026-08-26（數值經剛架有限元獨立驗證，與閉合式相符至 $10^{-8}$；向量圖腳本 `gen_SA-2024-4.py`，可重跑）*
