...
![SA-2006-4 題目重繪：剪力屋模型，剛性梁在 3m 高處，剛性矮牆高 1m，長柱 3m 兩根、短柱 2m 三根](figs/SA-2006-4-fig-1-frame.svg)

*圖 1　題目重繪（向量版）。這張圖攔的是柱長誤判：矮牆之上是 $2\text{ m}$，但**左側長柱從地面起算是 $2+1 = 3\text{ m}$** —— 代錯一個長度，兩個比值都會錯。*
...
| 知識點 | 說明 | 卡關? |
|---|---|:--:|
| 剪力屋（Shear Building）特性 | 橫梁剛度無限大 ＋ 忽略柱軸向變形 $\Rightarrow$ 所有柱頂有相同的側移 $\Delta$ 且**無轉角** | |
...
![SA-2006-4 短柱效應：五根柱的彎矩圖與側移，以及彎矩比 4/9、剪力比 8/27 的比較](figs/SA-2006-4-fig-2-short-column.svg)
...
|---|---|---|
| 2026-07-02 | 初版解析 | — |
| 2026-08-26 | 補兩張向量圖；§3.5 的破折號清單改為表格；主分類名稱與 `question_index.json` 對齊 | 所有數值（彎矩比 $4/9$、剪力比 $8/27$、$EI\Delta = 1800/97$、$M_{FA} = -12.37$、$V_{EL} = 27.84$）經剛架有限元（剛性梁以極大 $EI$ 模擬）驗證**全部正確**，解析本身無誤 |
