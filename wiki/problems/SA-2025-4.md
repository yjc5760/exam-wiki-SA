...
![SA-2025-4 題目重繪：軸測圖，b 在原點、ab 沿 x、bc 沿 y、cd 沿 x，a 與 d 為固定端，ω 沿 −z 作用於 bc](../../raw/solutions/SA-2025-4/figs/SA-2025-4-fig-1-grid.svg)

*圖 1　題目重繪（向量版）。把「桿件方向 ↔ 座標軸」的對應釘死：$ab$ 沿 $x$，所以繞 $x$ 的作用對它是**扭轉**、繞 $y$ 的是**彎曲**；$bc$ 沿 $y$ 則完全相反。這一組對應弄反，$GJ/L$ 與 $4EI/L$ 就會擺錯位置，整個 $3 \times 3$ 都失效。*
...
![SA-2025-4 自由度與對稱性：左為 b、c 各三個自由度共 6×6，右為對稱關係把獨立未知數降為三個](../../raw/solutions/SA-2025-4/figs/SA-2025-4-fig-2-dof-symmetry.svg)

*圖 2　自由度辨識與對稱性降階。雙箭頭是力矩／轉角向量（右手定則），左圖 6 個自由度、右圖用對稱性降到 3 個。這張圖攔的是「$\theta_{xb} = -\theta_{xc}$ 寫成同號」——一寫錯，$bc$ 桿的貢獻就會從 $2EI/L$ 變成 $6EI/L$。*
...
![SA-2025-4 節點 b 的三條平衡方程：ΣMx 由 ab 扭轉加 bc 彎曲、ΣMy 只剩 ab 彎曲、ΣFz 由 ab 剪力](../../raw/solutions/SA-2025-4/figs/SA-2025-4-fig-3-node-b.svg)

*圖 3　節點 $b$ 三條平衡方程的勁度來源，逐條標明「哪一根桿、以哪一種行為」提供。這張圖攔兩個最常見的失分：$J$ 沒換成 $2I$（實心圓桿 $J = I_x + I_y$），以及誤以為 $bc$ 桿有扭轉——$\theta_{yb} = \theta_{yc}$ 使它沿自身軸線的相對扭角為零。*
...
![SA-2025-4 等效懸臂梁驗核：左為 ab 桿側視圖與變形曲線，右為矩陣法與懸臂梁公式的四項數字對照](../../raw/solutions/SA-2025-4/figs/SA-2025-4-fig-4-cantilever-check.svg)

*圖 4　用一條與矩陣完全無關的路徑重算 $w_b$、$\theta_{yb}$、$M_{ya}$、$R_{za}$ 四項。這張圖攔的是「量級錯而不自知」：矩陣法算完若不驗算，$L$ 的次方或係數錯了在紙面上看不出來；$M_{ya} = \omega L^2/2$ 恰等於懸臂梁根部彎矩 $P \cdot L$，不是巧合而是必然。*

---

*圖解補繪：2026-08-16（向量圖，腳本 `gen_SA-2025-4.py`，可重跑；四張圖的符號式取自 §4，並在腳本中以一組具體數值逐條 assert）*


[互動圖形預覽](../../raw/solutions/SA-2025-4/SA-2025-4-matrix-viz.html)
